...
public class MultiUserMode extends WebSocketServer implements Runnable {
	...
	private static boolean landscapePosChanged = false; // tells whether a user already manipulated
														// landscape
	private final HashMap<String, Boolean> systemState; // tells if a system (systemID) is opened/closed
	private final HashMap<String, Boolean> nodeGroupState; // tells if a nodegroup (nodegroupID) is
															// opened/closed
	private final HashMap<String, ApplicationModel> apps; // maps applicationID to the application model
	...
	private void initializeLandscapeModel() {
		...
		/*
		 * final ExtensionAPIImpl coreAPI = ExtensionAPIImpl.getInstance(); final
		 * Landscape landscape = coreAPI.getLatestLandscape();
		 *
		 * final List<System> landscapeSystems = landscape.getSystems();
		 *
		 * // copy ids of systems and nodegroups to Hashmaps and initialize open state
		 * with // true for (final System LandscapeSystem : landscapeSystems) {
		 * systemState.put(LandscapeSystem.getId(), false); final List<NodeGroup>
		 * nodeGroups = LandscapeSystem.getNodeGroups(); for (final NodeGroup nodeModel
		 * : nodeGroups) { nodeGroupState.put(nodeModel.getId(), false); } }
		 */
			...
			for (int i = 0; i < queue.length(); i++) {
				...
				case "receive_component_update":
					if (JSONmessage.getBoolean("isFoundation")) {
						...
						apps.get(JSONmessage.getString("appID")).openComponent(JSONmessage.getString("componentID"));
					} else {
						apps.get(JSONmessage.getString("appID")).closeComponent(JSONmessage.getString("componentID"));
					}
